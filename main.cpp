...
#define D 55 //number of dimensions 53
#define datasize 581012 //581012
#define querysize 581012 //number of queries 5???
#define K 2 //k nearest neighbors 20
...
void diskread_double(std::string filename, double arrayA[][55]) // changed version
{
    double x;
	int colA = 0;
	int rowA = 0;
...
void diskread_double_query(std::string filename, double arrayA[][55]) // changed version
{
	double x;
	int colA = 0;
	int rowA = 0;
...
*/
	...
	for (int i = 0; i < querysize; i++)
	{
		...
		for (int j = 0; j < K; j++)
		{
			if (dshdist[j] <= gtdist[K - 1] + 0.01) { sumrecall++; localrecall++; } //0.001
			if (dshdist[j] - gtdist[j] < 0.01) continue;                            //0.001
...
int main()
{
    diskread_double("covtypeN.data", data);//changed version
    diskread_double_query("queryN.dat", query);//added function to read query
